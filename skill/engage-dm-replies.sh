...
1. Get ALL Twitter DM conversations visible in the sidebar (the script returns the full list, not only unread), INCLUDING the Message Requests tab (x.com/i/chat/requests), and write them to /tmp/twitter_threads.json:
   ...
   **URL form for request threads: keep `/i/chat/requests/<ids>` EXACTLY as scanned.** Pass it verbatim to read-conversation, ensure-dm, and send-dm. Do NOT strip the `/requests/` segment yourself: the canonical `/i/chat/<ids>` form does not resolve until the request is accepted (X bounces it to the requests list), so a hand-normalized chat_url breaks every later send. send-dm canonicalizes the stored chat_url automatically after the accepting reply goes through.
