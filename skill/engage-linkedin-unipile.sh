#!/usr/bin/env bash
# engage-linkedin-unipile.sh — STANDALONE Phase-1 proof of the UniPile LinkedIn
# post-commenting round trip (search -> comment).
#
# Phase 1 (this script): a manual, contained run that exercises scripts/
# linkedin_unipile.py end-to-end on real data so we can watch for a LinkedIn
# flag WITHOUT touching the live cron pipeline. It does NOT call the existing
# (chflags-locked) engage-linkedin.sh, does NOT acquire the linkedin-browser
# lock, and does NOT gate on the killswitch — there is no browser here.
#
# Default behavior is DRY RUN: it probes credentials, searches, and prints the
# comment it *would* post. Posting a real comment is visible to real people, so
# it only happens when you pass --post AND --text.
#
# Usage:
#   ./engage-linkedin-unipile.sh --keywords "ai agents" --date-posted past_week --limit 5
...
log "=== UniPile LinkedIn engage (Phase 1, standalone) ==="
