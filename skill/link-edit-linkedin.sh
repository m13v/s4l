...
7. Navigate to the post URL via the bh_run browser (new_tab/goto_url + wait_for_load), find our comment, click the three-dot menu on it (click_at_xy), click "Edit". CRITICAL caret placement: LinkedIn's edit box is a contenteditable that puts the caret wherever you click, and type_text inserts at the caret, so you MUST move the caret to the VERY END of the existing comment text before typing or the link gets inserted at the BEGINNING. After clicking into the edit box, jump the caret to the end: press_key "Meta+ArrowDown" then press_key "End" (macOS); if the caret does not reach the end, press_key "End" again or press_key "Control+End". Only once the caret is confirmed at the end, type_text the blank-line separator followed by the wrapped link text so it is APPENDED after the original content. Save, then verify with capture_screenshot + Read the PNG: confirm the link sits at the END of the comment, not the start. If it landed at the start, re-edit and fix the placement before moving on.
