...
3. DRAFT HANDLING (existing vs fresh):
   ...
   - Otherwise: draft a reply using the best engagement style. One sentence or a tight fragment — aim for under 100 chars. NEVER em dashes. Apply the matched project's \`voice\` block from ALL_PROJECTS_JSON: follow voice.tone, never violate voice.never, mirror voice.examples / voice.examples_good when present.
...
4. EMIT one entry in the structured 'candidates' array with these fields:
   ...
   - new_style (object, REQUIRED iff INVENT mode produced a new name; OMIT or set null otherwise): {description (string), example (string), why_existing_didnt_fit (string), note (string, optional), target_chars (integer, REQUIRED)}. Same shape you passed to --new-style in step 3a. The post pipeline reads this and POSTs to /api/v1/engagement-styles/registry so the new style lands in engagement_styles_registry alongside Reddit/GitHub/Moltbook inventions. target_chars is the comment length THIS new style wins at, in characters: bias SHORT (the top human replies that win run ~45-90 chars; 250 is only a hard ceiling, never a target). Pick the smallest length that still lets the style land its move — a one-liner style ~45, a story-arc style up to ~180, never above 220.
