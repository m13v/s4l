...
def send(A, MSG, UNIQ):
    ...
    if prof.get('suspended') or not prof.get('hasMsg'):
        out["status"]="no_dm"; print(_j.dumps(out)[:1700]); return
    ...
    if not rect:
        out["status"]="no_composer"; out["url"]=page_info().get('url'); print(_j.dumps(out)[:1700]); return
    ...
    if (val or "").strip()!=MSG:
        out["status"]="type_mismatch"; out["len"]=len(val or ""); out["url"]=page_info().get('url'); print(_j.dumps(out)[:1700]); return
