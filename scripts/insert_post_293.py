...
DB = env["DATABASE_URL"]
caption = Path(os.path.expanduser("~/social-autoposter/mixer/remotion/out/post-293.caption.txt")).read_text()
video_path = os.path.expanduser("~/social-autoposter/mixer/remotion/out/post-293.mp4")

# 4 clips, each exactly 2.0s encoded, filling a 2.0s slot (no speedup at slot time, speedup=1.0)
clips = [("mixer/tlh-4-4.mp4",2.0),("mixer/tlh-21-3.mp4",2.0),("mixer/tlh-6-4.mp4",2.0),("mixer/tlh-62-1.mp4",2.0)]
source_clips=[]; t=0.0
...
overlays_txt=["i did brand strategy for 12 years.","an agent wrote the deck in an afternoon.","cold coffee. kitchen. 1am.","the deck was never the job."]
overlays=[{"order":i,"text":x,"start_sec":i*2.0,"end_sec":i*2.0+2.0,"dur_sec":2.0} for i,x in enumerate(overlays_txt)]
metadata={"composition_id":"TLH-lesson-293","format":"TLH","theme":"ai","theme_angle":"ai-killed-the-brand-strategist","theme_label":"brand strategist defeat-flip","clip_count":4,"overlay_count":4,"caption_style":"ig_defeat_flip_arc","description_style":"ig_defeat_flip_arc","source_repo":"social-autoposter/mixer","engagement_style":"ig_defeat_flip_arc"}
audio_source="local:"+os.path.expanduser("~/social-autoposter/mixer/audio/track-022_DXMgHxNjR7Y.m4a")

conn=psycopg2.connect(DB); cur=conn.cursor()
cur.execute("SELECT id,post_number,status FROM media_posts WHERE variant_id=%s OR post_number=%s",("lesson-293",293))
...
""", (293,"lesson-293",video_path,audio_source,caption,json.dumps(source_clips),json.dumps(overlays),json.dumps(metadata)))
